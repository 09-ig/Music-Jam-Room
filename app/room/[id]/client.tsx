...
export function RoomClient({ roomId, user }: RoomClientProps) {
  ...
  const handleSeek = (time: number) => {
    socket?.emit("seekSong", { roomId, time })
  }
        ...
        <div className="space-y-4">
          {songRequests.map((request) => (
            <SongRequestItem
              ...
              onApprove={() => socket?.emit("approveSong", { roomId, songId: request.id })}
              onReject={() => socket?.emit("rejectSong", { roomId, songId: request.id })}
            />
          ))}
        </div>
      </div>
